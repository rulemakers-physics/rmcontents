...
interface ResultData {
  ...
  score: number;
  phone: string;
  answers: Record<string, number>; // DB에는 string key로 저장됨
  results: Record<string, boolean>;
}

interface ExamStat {
  totalTakers: number;
  questionStats: Record<string, number>; // 문항별 정답률(%)
}
...
export default function MarketingResultPage() {
  ...
  // 2. 전체 통계 집계 (클라이언트 사이드 간이 집계)
  // * 주의: 실제 운영 시에는 데이터 양이 많으면 Cloud Functions 등으로 서버 집계를 권장합니다.
  const fetchStats = async (examId: string) => {
    try {
      ...
      const qCounts: Record<string, number> = {}; // 문항별 정답자 수
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 font-sans">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <DocumentArrowDownIcon className="w-6 h-6 text-blue-600" /> 해설지 다운로드
          </button>
          <button 
            onClick={() => alert("준비 중입니다.")}
            className="flex items-center justify-center gap-2 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors font-bold text-slate-700"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-center">
              ...
              <tbody className="divide-y divide-slate-100">
                {/* 1번부터 20번까지 루프 (임시) - 실제로는 examData의 problems 매핑 필요 */}
