...
export default function ClassDetailModal({ classData, onClose }: Props) {
  ...
  const [newParentPhone, setNewParentPhone] = useState(""); // [신규] 학부모 연락처
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "students"), {
        ...
        school: newSchool,
        phone: newPhone,
        parentPhone: newParentPhone, // [신규] 저장
      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">학생 명단 관리</p>
          </div>
          ...
          <form onSubmit={handleAddStudent} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              ...
              <div className="md:col-span-1">
                <button 
                  type="submit"
                  className="w-full h-[42px] bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold text-sm flex items-center justify-center gap-1 shadow-sm active:scale-95 transition-all"
                >
          ...
          {students.length === 0 && !isLoading ? (
            ...
            <div className="space-y-3">
              {students.map((student) => (
                <div key={student.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 transition-colors group gap-3">
